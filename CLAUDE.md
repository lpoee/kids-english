...
## Lessons / gotchas
Do not equate successful rendering or pytest output with visual approval: review every Social turn at start/middle/end and keep branch outcomes visibly distinct. Scene plans must conserve full prop inventory, preserve ownership and branch continuity, and show temporal actions such as building, chasing, opening, stopping, and independent repair rather than starting on the final result. Requests must not move the prop; fixed equipment must stay fixed; seated children need hips on the seat and feet below it. Mobile audio must continue using the persistent player and removable one-shot `ended` handlers so iOS does not lose the tap or duplicate playback. Keep card-menu targets at least 48px tall, place submenus below the main row, and hide empty submenu overlays so iPad taps reach the intended button.
